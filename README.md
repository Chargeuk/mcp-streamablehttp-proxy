...
You can override the per-request timeout (default 300s) per request. Two options are supported:

1. Add `modelTimeoutOverride` inside the `params.arguments` payload (preferred; automatically stripped before forwarding to the stdio MCP server).
2. Add `_meta.timeoutSeconds` inside `params` (legacy support).

Example using `modelTimeoutOverride`:
